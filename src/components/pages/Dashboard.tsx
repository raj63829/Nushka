    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-sage-800 mb-4">Please Sign In</h2>
          <p className="text-sage-600">You need to be logged in to access your dashboard.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <h1 className="text-3xl font-bold text-sage-800 mb-8">My Dashboard</h1>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-sm border border-sage-100 p-6">
              <div className="flex items-center space-x-3 mb-6">
                ...
                <div>
                  <h3 className="font-semibold text-sage-800">{authState.user?.name}</h3>
                  <p className="text-sm text-sage-600">{authState.user?.email}</p>
                </div>
              </div>
          ...
          <div className="lg:col-span-3">
            <div className="bg-white rounded-lg shadow-sm border border-sage-100 p-6">
              {activeTab === 'profile' && (
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-semibold text-sage-800">Profile Information</h2>
                    <button
                  ...
                  {isEditing ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="text-sage-800">{authState.user?.email}</p>
                      </div>
                      <div>
              ...
              {activeTab === 'addresses' && (
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-semibold text-sage-800">Saved Addresses</h2>
                    <button
                    ...
                    <div className="mb-6 p-4 border border-sage-200 rounded-md">
                      ...
                      <form onSubmit={handleAddressSubmit} className="grid grid-cols-2 gap-4">
                        ...
                        <div className="col-span-2">
                          <label className="block text-sm font-medium text-sage-700 mb-2">Address Line 1</label>
                          <input
                            type="text"
                        ...
                        <div className="col-span-2">
                          <label className="block text-sm font-medium text-sage-700 mb-2">Address Line 2</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="flex items-center space-x-2">
                            ...
                            />
                            <span className="text-sm text-sage-700">Set as default address</span>
                          </label>
                        </div>
                  ...
                  <div className="space-y-4">
                    {addresses.map((address) => (
                      <div key={address.id} className="border border-sage-200 rounded-md p-4">
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            <p className="text-sage-600 text-sm">Phone: {address.phone}</p>
                            {address.is_default && (
              ...
              {activeTab === 'orders' && (
                <div>
                  <h2 className="text-xl font-semibold text-sage-800 mb-6">My Orders</h2>
                  <div className="space-y-4">
                    {orders.length === 0 ? (
                      <p className="text-sage-600">No orders found.</p>
                    ) : (
                      orders.map((order) => (
                        <div key={order.order_id} className="border border-sage-200 rounded-md p-4">
                          <div className="flex items-center justify-between mb-4">
                            <div>
                              <h4 className="font-medium text-sage-800">Order #{order.order_id.slice(-8)}</h4>
                              <p className="text-sm text-sage-600">
                                {new Date(order.created_at).toLocaleDateString()}
                              </p>
                            </div>
                            <div className="text-right">
                              <p className="font-semibold text-sage-800">₹{order.total_amount.toLocaleString()}</p>
              ...
              {activeTab === 'wishlist' && (
                <div>
                  <h2 className="text-xl font-semibold text-sage-800 mb-6">My Wishlist</h2>
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {cartState.wishlist.map((item) => (
                      <div key={item.id} className="border border-sage-200 rounded-md p-4">
                        <img src={item.images[0]} alt={item.name} className="w-full h-32 object-cover rounded-md mb-3" />
                        <h4 className="font-medium text-sage-800 mb-2">{item.name}</h4>
                        <p className="text-sage-600 text-sm mb-2">₹{item.price.toLocaleString()}</p>
              ...
              {activeTab === 'settings' && (
                <div>
                  <h2 className="text-xl font-semibold text-sage-800 mb-6">Account Settings</h2>
                  <div className="space-y-6">
                    <div>
                      <h3 className="text-lg font-medium text-sage-800 mb-3">Security</h3>
                      <button className="bg-sage-600 text-white px-4 py-2 rounded-md hover:bg-sage-700 transition-colors">
                        Change Password
                      </button>
                    </div>
                    <div>
                      <h3 className="text-lg font-medium text-sage-800 mb-3">Notifications</h3>
                      <div className="space-y-2">
                        <label className="flex items-center space-x-2">
                          <input type="checkbox" className="text-sage-600" defaultChecked />
                          <span className="text-sage-700">Email notifications for orders</span>
                        </label>
                        <label className="flex items-center space-x-2">
                          <input type="checkbox" className="text-sage-600" defaultChecked />
                          <span className="text-sage-700">SMS notifications for delivery</span>
                        </label>
                        <label className="flex items-center space-x-2">
                          <input type="checkbox" className="text-sage-600" />
                          <span className="text-sage-700">Marketing emails</span>
                        </label>
                      </div>
                    </div>
                  </div>
